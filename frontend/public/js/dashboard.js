...
function renderizarContadores(dados) {
    ...
    const totalHoje = obterNumeroExibicao(
        dados.totalHoje
    );
    ...
    if (
        dados.gravidadeHoje &&
        typeof dados.gravidadeHoje === 'object'
    ) {

        quantidadeBaixa = obterNumeroExibicao(
            dados.gravidadeHoje.baixa
        );

        quantidadeMedia = obterNumeroExibicao(
            dados.gravidadeHoje.media
        );

        quantidadeAlta = obterNumeroExibicao(
            dados.gravidadeHoje.alta
        );
...
function criarLinhaAtendimento(atendimento) {
    ...
    const colunaPaciente = document.createElement(
        'td'
    );

    colunaPaciente.className =
        'py-4 pr-3';


    const nomeFuncionario = document.createElement(
        'p'
    );

    nomeFuncionario.className =
        'font-semibold text-azulEscuro';

    nomeFuncionario.textContent = obterTextoExibicao(
        atendimento.nome,
        'Funcionário sem nome'
    );


    const dadosFuncionario = document.createElement(
        'p'
    );

    dadosFuncionario.className =
    ...
    dadosFuncionario.textContent =
        'Matrícula: ' +
        matricula;


    const queixa = document.createElement(
        'p'
    );
    ...
    const colunaGravidade = document.createElement(
        'td'
    );

    colunaGravidade.className =
        'py-4 text-center';


    const gravidade = obterTextoExibicao(
        atendimento.gravidade,
        'Não informada'
    );

    const badgeGravidade = document.createElement(
        'span'
    );
    ...
    colunaGravidade.appendChild(
        badgeGravidade
    );


    const colunaData = document.createElement(
        'td'
    );

    colunaData.className =
        'py-4 text-center text-slate-500 text-xs whitespace-nowrap';

    colunaData.textContent = formatarDataHora(
...
function renderizarUltimosAtendimentos(atendimentos) {
    ...
    atendimentos.forEach(
        function (atendimento) {
            ...
            if (
                atendimento.funcionario_matricula === null ||
                atendimento.funcionario_matricula === undefined
            ) {
...
function renderizarSetoresCarregando() {
    ...
    mensagem.className =
        'text-slate-400 text-center py-6 text-sm';
...
function criarItemSetor(setor) {
    ...
    const nomeSetor = document.createElement(
        'span'
    );
    ...
    nomeSetor.textContent = obterTextoExibicao(
        setor.setor,
        'Não informado'
    );


    const quantidade = document.createElement(
        'span'
    );

    quantidade.className =
        'font-bold text-azulEscuro bg-blue-100 px-3 py-1 rounded-full text-xs';

    quantidade.textContent = String(
        obterNumeroExibicao(
            setor.quantidade
        )
    );
...
function renderizarRankingSetores(setores) {
    ...
    if (!Array.isArray(setores)) {
        ...
        const mensagemErro = document.createElement(
            'div'
        );

        mensagemErro.className =
            'text-red-500 text-center py-6 text-sm';

        mensagemErro.textContent =
            'Não foi possível interpretar os setores.';
    ...
    if (setores.length === 0) {
        ...
        const mensagemVazia = document.createElement(
            'div'
        );

        mensagemVazia.className =
            'text-slate-400 text-center py-6 text-sm';

        mensagemVazia.textContent =
            'Sem atendimentos por setor neste mês.';
...
function mostrarErroDashboard(mensagem) {
    ...
    const setores = document.getElementById(
        'lista-setores'
    );
    ...
    if (setores) {
        ...
        mensagemErro.className =
            'text-center text-sm text-red-500 py-6';
...
async function carregarDadosDashboard() {
    ...
    try {
        ...
        renderizarUltimosAtendimentos(
            dados.ultimosAtendimentos
        );

        renderizarRankingSetores(
            dados.atendimentosPorSetor
        );
