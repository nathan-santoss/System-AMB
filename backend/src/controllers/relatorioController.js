...
import { montarRelatorioPop, obterLiderancas, formatarDataRelatorio } from '../services/relatorioPopService.js';
...
export async function imprimirFichaAtendimento(req, res) {
    try {
        ...
        if (!matriculaEhValida(matricula) || !/^\d+$/.test(req.params.id) || !Number.isSafeInteger(id) || id <= 0) {
        ...
        const registro = await Atendimento.findOne({ where: { id_atendimento: id, funcionario_matricula: matricula } });
        if (!registro) return res.status(404).json({ erro: 'Atendimento não encontrado para este funcionário.' });
        ...
        return res.render('ficha-atendimento-impressao', {
            funcionario, atendimento, emitidoEm: new Date(), formatarData: formatarDataRelatorio
        });
    } catch (erro) {
