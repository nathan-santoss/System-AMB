...
function formatarPlanilha(planilha) {
    ...
    planilha.autoFilter = { from: { row: 1, column: 1 }, to: { row: Math.max(1, planilha.rowCount), column: planilha.columnCount } };
}
...
export function criarPlanilhaFuncionario(relatorio) {
    ...
    funcionario.columns = [{ header: 'Campo', key: 'campo', width: 32 }, { header: 'Valor', key: 'valor', width: 65 }];
    const f = relatorio.funcionario;
    const dados = [
        ['Nome', f.nome], ['Matrícula', String(f.matricula)],
        ['Cargo', f.cargo], ['Setor', f.setor], ['Núcleo', f.nucleo],
    ...
    dados.filter(([, valor]) => valor !== null && valor !== undefined && valor !== '')
    ...
    const colunas = [
        ...
        { header: 'Entrada (Brasília)', key: 'entrada', width: 24, style: { numFmt: 'dd/mm/yyyy hh:mm' } },
        { header: 'Saída (Brasília)', key: 'saida', width: 24, style: { numFmt: 'dd/mm/yyyy hh:mm' } },
    ...
    for (const a of relatorio.atendimentos) {
        atendimentos.addRow({
            id: a.id_atendimento, matricula: String(f.matricula),
            entrada: dataExcel(a.data_hora_entrada), saida: dataExcel(a.data_hora_saida),
            situacao: a.data_hora_saida ? 'Finalizado' : 'Em aberto',
            pressao: a.pressao_arterial,
            temperatura: a.temperatura == null ? null : Number(a.temperatura),
            gravidade: a.gravidade, queixa: a.queixa_principal,
            acao: a.acao_tomada, encaminhamento: a.local_encaminhamento,
            ...Object.fromEntries(a.liderancas.map(l => [l.cargo, l.nome]))
        });
    }
    const alergias = workbook.addWorksheet('Alergias atuais');
    alergias.columns = [{ header: 'Matrícula', key: 'matricula', width: 22 }, { header: 'Alergia cadastrada na emissão', key: 'descricao', width: 70 }];
